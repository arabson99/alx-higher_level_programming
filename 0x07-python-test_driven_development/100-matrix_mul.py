...
def matrix_mul(m_a, m_b):
	...
	Raises:
		...
		TypeError: if either m_a or m_b has different_size rows.
	...
	if not all(len(row) == len(m_b[0]) for row in m_b):
		raise TypeError("eacg row of m_b should be of the same size")

	if len(m_a[0]) != len(m_b):
		raise ValueError("m_a and m_b can't be multiplied")

	result_matrix = [[0] * len(m_b[0]) for i in range(len(m_a))]
